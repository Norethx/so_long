...
/*   Updated: 2025/09/09 19:21:25 by rgomes-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_fill_scene(t_game *game)
{
	...
	mlx = ft_mlx_obj(15, game);
	if (!mlx)
		return (NULL);
	img[2] = 0;
	ft_build_floor(game);
	ft_build_walls(game);
	ft_render_child(game, 'P', ft_mlx_obj(18, NULL), ft_mlx_obj(15, NULL));
	ft_render_child(game, 'X', ft_mlx_obj(19, NULL), ft_mlx_obj(15, NULL));
...
static void	ft_render_child(t_game *game, char c, t_player *child, mlx_t *mlx)
{
	...
	while (i[0] < game->size.y)
	{
		...
		while (i[1] < game->size.x)
		{
			...
			while (game->map[i[0]][i[1]] == c && i[2] < 8)
			{
				mlx_image_to_window(mlx, child->idle[i[2]], i[1] * WIDTH, i[0]
					* WIDTH);
				mlx_image_to_window(mlx, child->run[i[2]], i[1] * WIDTH, i[0]
					* WIDTH);
				if (i[2] != 0)
